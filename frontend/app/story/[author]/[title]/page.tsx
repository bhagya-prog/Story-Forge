...
import { Heart, MessageCircle, Share2, Bookmark, Eye, Clock, Send, ThumbsUp, Edit, Trash2, GitFork } from "lucide-react"
import { storyService, commentService, type Story, type Comment } from "@/lib/backend-services"
...
export default function StoryPage() {
  ...
  const fetchStory = async () => {
    setLoading(true)
    try {
      // For now, we'll need to search by title since we don't have the ID in the URL
      // In a real app, you'd want to use story ID in the URL
      const stories = await storyService.getAllStories();
      const decodedTitle = decodeURIComponent(params.title as string).replace(/-/g, ' ');
      ...
      if (foundStory) {
        ...
        // If not found, create a placeholder story
        setStory({
          id: 'not-found',
          title: decodedTitle.replace(/\b\w/g, l => l.toUpperCase()),
          authors: [decodeURIComponent(params.author as string).replace(/-/g, ' ')],
          description: 'Story content not found.',
          tags: [],
          likes: 0,
          forks: 0,
          views: 0,
  ...
  const fetchComments = async () => {
    if (!story || story.id === 'not-found') return;
    
    try {
      const response = await commentService.getCommentsByStory(story.id);
      setComments(response.comments);
    } catch (error) {
      console.error("Failed to fetch comments:", error);
      // Keep mock comments as fallback for now
  ...
  const handleLike = async () => {
    ...
    try {
      const result = await storyService.likeStory(story.id);
      setStory(prev => prev ? { ...prev, likes: result.likes } : null);
      setIsLiked(!isLiked);
      toast({
        title: "Success",
        description: isLiked ? "Story unliked!" : "Story liked!",
      });
    } catch (error) {
  ...
  const handleFork = async () => {
    ...
    const forkTitle = prompt('Enter a title for your fork:', `${story.title} (Fork)`);
    if (!forkTitle) return;
    
    try {
      const forkedStory = await storyService.forkStory(story.id, {
        title: forkTitle,
        isPrivate: false
      });
      ...
      toast({
        title: "Success",
        description: "Story forked successfully!",
      });
      
      // Optionally redirect to the forked story
      // window.location.href = `/story/${forkedStory.authors[0]}/${forkedStory.title.toLowerCase().replace(/\s+/g, '-')}`;
    } catch (error) {
      console.error('Fork story error:', error);
      toast({
        title: "Error",
        description: "Failed to fork story",
        variant: "destructive",
      });
    }
  };
  ...
  const handleComment = async () => {
    if (!newComment.trim() || !story || story.id === 'not-found') return;
    
    try {
      const comment = await commentService.createComment(story.id, {
        content: newComment
      });
                ...
                <div className="flex items-center space-x-3 mb-4">
                  <Avatar>
                    <AvatarFallback className="bg-electric-blue text-white">
                      {story.authors[0]?.charAt(0)?.toUpperCase() || 'A'}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    <p className="font-medium text-gray-900">{story.authors[0] || 'Unknown Author'}</p>
                  ...
                  <div key={comment._id} className="flex space-x-4">
                    <Avatar>
                      <AvatarFallback className="bg-gray-200 text-gray-600">
                        {comment.author.avatar || comment.author.username.charAt(0).toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
                    <div className="flex-1">
