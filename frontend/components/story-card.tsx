...
export function StoryCard({
  ...
  const handleCardClick = () => {
    ...
    // Handle case where authors array might be empty or contain non-string values
    const firstAuthor = authors && authors.length > 0 && typeof authors[0] === 'string' 
      ? authors[0] 
      : 'unknown-author'
    
    const authorSlug = firstAuthor.toLowerCase().replace(/\s+/g, "-")
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-600 mt-1">by {
              Array.isArray(authors) && authors.length > 0 
                ? authors.filter(author => typeof author === 'string').join(", ") || "Unknown Author"
                : "Unknown Author"
            }</p>
          </div>
          {coverImage && (
